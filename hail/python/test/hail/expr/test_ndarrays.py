...
def test_mixed_ndarray_pipelines():
    ...
    ht = ht.annotate(a=8)
    ht = ht.annotate(map_of_matmul_of_maps=((ht.ones_square_qr[0] - ht.a) @ (ht.ones_square + (ht.ones_square * 8))) + hl.nd.array(hl.range(10)))
    ht = ht.annotate(flattend_with_ranges=hl.nd.array(hl.range(10).map(lambda i: hl.range(10).map(lambda j: ht.map_of_matmul_of_maps[i, j] + i + j))))
